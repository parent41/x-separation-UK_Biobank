% Don't think this is necessary if we do dcm2nii

path = './tmp2/';

addpath('./x-separation-UK_Biobank');
...
% Need to parallelize this loop for efficiency
for i = 1:size(flist,1)
    disp(i)
    fname=flist(i).name;
    info1 = dicominfo([di fname]);
...
mkdir([path 'NII']);
mkdir([path 'NII/MAG_TE1']);
mkdir([path 'NII/MAG_TE2']);
for i = 1:coils_num
    disp(i)
    niftiwrite(rot90(img_data(:,:,1:slices_num,i),-1), [path 'NII/MAG_TE1/' num2str(i) '.nii'], nii_info, 'Compressed', true);
    niftiwrite(rot90(img_data(:,:,slices_num+1:end,i),-1), [path 'NII/MAG_TE2/' num2str(i) '.nii'], nii_info, 'Compressed', true);
end
%% 
