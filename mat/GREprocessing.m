...
function GREprocessing(path)
    % path = './tmp_chisep_2';
    % path = './chisep_sub-1000011_ses-2';

    mkdir([path '/GREprocessing']);

    rmpath(genpath('./x-separation-UK_Biobank/mat/utils/STISuite_V3.0/'));
    addpath(genpath('./x-separation-UK_Biobank/mat/utils/STISuite_V3.0/'));
    ...
    TE1 = '0.00942';
    TE2 = '0.01970';
    fn_pha1 = dir([path '/swi_nii/*cH*_e1_ph.nii']);

    num_channels = length(fn_pha1);

    for j = 1:num_channels
        phs1(:, :, :, j) = double(niftiread([path '/swi_nii/' fn_pha1(j).name]));
    end

    dim = size(phs1); 
    M1 = zeros(dim);
    fn_mag1 = dir([path '/swi_nii/*cH*_e1.nii']);

    for j = 1:num_channels
        M1(:, :, :, j) = double(niftiread([path '/swi_nii/' fn_mag1(j).name]));
    end
    ...
    phs2 = zeros(dim);
    fn_pha2 = dir([path '/swi_nii/*cH*_e2_ph.nii']);

    for j = 1:num_channels
        phs2(:, :, :, j)=double(niftiread([path '/swi_nii/' fn_pha2(j).name]));
    end

    M2 = zeros(dim);
    fn_mag2 = dir([path '/swi_nii/*cH*_e2.nii']);

    for j = 1:num_channels
        M2(:, :, :, j) = double(niftiread([path '/swi_nii/' fn_mag2(j).name]));
    end
    ...
    % What does this bit do? Not in UKB script

    voxel_size = [0.8, 0.8, 3];
    matrix_size = size(S2(:,:,:,1));
    voxelsize_new = [1.05 1 3]; % vs UKB: Downsampling to 1mm in-plane res
    ...
    disp('Coil combine with MCPC-3D-S')
    ...
    % ref_file = dir([path '/swi_nii/*_cH25_e1.nii.gz']); % In UKB script, uses mask info
    % nii_info = niftiinfo([path '/t1_flair_mask/mask_SWI_res.nii.gz']); % For some reason, not in T1w space...
    % nii_info = niftiinfo('./x-separation-UK_Biobank/template_forchisep_113mm.nii.gz');
    ...
    niftiwrite(hip_abs, [path '/GREprocessing/hip_abs.nii'], nii_info, 'Compressed', true);
    niftiwrite(hip_angle, [path '/GREprocessing/hip_angle.nii'], nii_info,'Compressed', true);

    %% prelude unwrapping

    disp('-  Prelude unwrapping')

    system(['bet2 ' path '/GREprocessing/hip_abs.nii.gz ' path '/GREprocessing/QSM -m -f 0.35']);
    system(['prelude -a ', path, '/GREprocessing/hip_abs.nii.gz -p ', ...
            path, '/GREprocessing/hip_angle.nii.gz -u ', path, ...
            '/GREprocessing/hip_uw.nii.gz -m ', path, '/GREprocessing/QSM_mask.nii.gz']);

    %% R2star mapping
    disp('-  Calculate R2* and PD')

    unwrappedHip=double(niftiread([path '/GREprocessing/hip_uw.nii.gz']));
    ...
    save([path '/GREprocessing/r2starimg_svd_2e.mat'],'R2star_img','pd')
    niftiwrite(r2star, [path '/GREprocessing/r2star.nii'], nii_info, 'Compressed', true);
    niftiwrite(pd, [path '/GREprocessing/pd.nii'], nii_info, 'Compressed', true);
    ...
    mask = double(niftiread([path '/GREprocessing/QSM_mask.nii.gz']));
    nii = angle(combined1) .* mask;

    nii_info.Datatype = 'single';
    niftiwrite(single(nii), [path '/GREprocessing/PHASE_TE1.nii'], nii_info, ...
            'Compressed',true);
    ...
    niftiwrite(single(nii), [path '/GREprocessing/PHASE_TE2.nii'], nii_info, ...
            'Compressed',true);
    ...
    disp('Extract DICOM info and update mask')
    ...
    % Many differences with UKB processing for this bloc
    ...
    disp('V-SHARP and Dipole inversion')
    ...
    save([path '/GREprocessing/for_xsep.mat'],'dB_vsf','r2star','Mag_brain','mask_vsf','TE','B0','H','voxsz','path','voxelsize_new','voxel_size')
    ...
    qsm_iLSQR_vsf = QSM_iLSQR(dB_vsf, mask_vsf, 'TE', TE, 'B0', B0, ...
                ...
                'voxelsize', voxsz);
    niftiwrite(single(qsm_iLSQR_vsf * 1000), [path '/GREprocessing/QSM.nii'], ...
            nii_info, 'Compressed', true);

    x_sa = qsm_iLSQR_vsf/2/pi/123;
    save([path '/GREprocessing/QSM.mat'],'qsm_iLSQR_vsf','x_sa')
